	...
	\chapter{Présentation de l'entreprise}
		\section{L'\acrshort{UHA}}
			\subsection{Composition}
				\paragraph*{}
					L'\gls{UHA} est une université française composée de 5 campus sur 2 villes~\cite{UHA_Organisation}, les campus Illberg, Collines et Fonderie à Mulhouse et les campus Biopôle et Grillenbreit à Colmar. Elle compte 4 facultés, la \gls{FLSH}, la \gls{FSESJ}, la \gls{FST} et la \gls{FMA}.
				\paragraph*{}
					Elle compte aussi 2 écoles d'ingénieurs, l'\gls{ENSCMu} et l'\gls{ENSISA}, 2 \gls{IUT}, un à Mulhouse et un à Colmar et plusieurs services de formation et certifications lui sont rattachés (voir annexe \ref{sec:uha_formation}).
		...
		\section{L'\acrshort{IRIMAS}}
			...
			\subsection{Équipes de recherche}
				\paragraph*{}
					...
					\begin{figure}[H]
						...
						\caption{Organigramme de l'\acrshort{IRIMAS} en 2019~\cite{IRIMAS_Organigramme}}%
			...
			\subsection{Équipe \acrshort{OMeGA}}
				...
				\paragraph*{}
					Selon \cite{IRIMAS_OMeGA}, le groupe travaillant sur le premier axe de recherche s'intéresse à l'algorithmique pour l'intelligence artificielle en développant de nouveaux algorithmes hybrides pour résoudre des problèmes d'optimisation mono-objectif ou multi-objectif de natures continues, discrètes et combinatoires, en prenant en compte l'aspect dynamique quand cela est nécessaire. Ils étudient également l'adaptation et l'implémentation de ces futurs algorithmes hybrides sur des clusters hétérogènes de machines GPU.
				\paragraph*{}
					Le groupe qui s'intéresse au deuxième axe de recherche travaille sur l'analyse de données visuelles et géométriques (images, vidéo, nuages de points, courbes,\ldots). Pour répondre à des problématiques de visualisation et d'analyses de scènes, ils développent des algorithmes et des concepts pour résoudre des problèmes de nature géométrique qui apparaissent dans de nombreuses disciplines de l'informatique ainsi que des techniques d'extraction d'information dans des données visuelles.
				\paragraph*{}
	...
	\chapter{Présentation du stage}\label{sec:presentation_stage}
		\section{Contexte du stage}
			...
			\subsection{\acrshort{SCP} et \acrshort{USCP}}\label{sec:SCP_et_USCP}
				...
				\paragraph*{}
					...
					\BAnewcolumntype{m}{>{\centering\arraybackslash\hspace{0pt}}p{15pt}}%
					\begin{equation}
						\begin{blockarray}{ccccccccc}
							& s_1 & s_2 & s_3 & s_4 & s_5 & s_6 & s_7\\
							\begin{block}{c(mmmmmmm)c}
								    & 0 & 1 & 1 & 0 & 0 & 0 & 1 & u_1\\
								    & 0 & 0 & 1 & 0 & 1 & 1 & 0 & u_2\\
								A = & 1 & 1 & 0 & 1 & 0 & 1 & 0 & u_3\\
								    & 0 & 0 & 0 & 0 & 1 & 1 & 1 & u_3\\
		...
		\section{Objectifs du stage}
			...
			\subsection{Objectifs supplémentaires}
				\paragraph*{}
					Après que la première version de notre algorithme ait été conçue et implémentée (voir section \ref{sec:memetic_algorithm} pour le détail des différentes versions de l'algorithme) et que des bon résultats ont été obtenus, il a été décidé de soumettre à la conférence \acrshort{ROADEF2020} qui est le ``\acrlong{ROADEF2020}''~\cite{ROADEF2020}.
				\paragraph*{}
		...
		\section{Organisation du stage}\label{sec:organisation_stage}
			...
			\subsection{Déroulement du stage}\label{sec:planning}
				...
				\paragraph*{}
					...
					\begin{itemize}
						\item La lecture d'articles scientifique et de documentation: la revue de la littérature initiale, la documentation des librairies utilisées pour le développement des programmes, la documentation pour l'utilisation du cluster \gls{HPC},\ldots
	...
	\chapter{Travail réalisé}\label{sec:work}
		\section{Revue de la littérature}\label{sec:review_litterature}
			...
			\subsection{\acrshort{RWLS}}\label{sec:RWLS}
				...
				\paragraph*{}
					\gls{RWLS} commence par générer une solution greedy, puis fonctionne par étapes de manière itérative, les étapes sont découpées en sous-étapes tel que représenté sur la figure \ref{fig:rwls_alg}.
					\begin{figure}[H]
						\begin{tikzpicture}[
							remember picture,
							x=1pt,
							y=1pt,
							/utils/exec={\sffamily},
							font=\fontsize{10.5pt}{9.5pt}\selectfont,
						]
			...
			\subsection{Instances de benchmark}
				...
				\end{table}
				\paragraph*{}
					La table~\ref{tab:instances_sts} contient les détails des 11 instances issues des \gls{STS} provenant de~\cite{Fulkerson1974}. Ces instances se construisent à partir de l'instance de taille précédente en commençant avec \instance{STS9}, elles ont une structure régulière avec chaque ligne couverte par exactement 3 colonnes. Les instances \instance{STS9} à \instance{STS729} sont très fréquemment utilisées, \cite{Gao2015} est le premier article à utiliser \instance{STS1215}, on peut aussi noter qu'il n'utilise pas les instances de \instance{STS9} à \instance{STS135} car ces dernières sont très faciles à résoudre. Pour nos benchmarks j'ai commencé par les instances classiques et sur fin du stage j'ai été ajoutés les instances \instance{STS1215} et \instance{STS2187}.
				\begin{table}[H]
					\centering
		...
		\section{Implémentation de \acrshort{RWLS}}\label{sec:impl_RWLS}
			...
			\subsection{Implémentation et optimisation}
				...
				\paragraph*{}
					Cette implémentation n'était pas parallélisable mais a permis de gagner en performances et de pouvoir traiter les instances \instance{RAIL}. De nombreuses autres optimisations moins majeures du code ont été réalisées, notamment au cours du développement de l'algorithme mémétique mais ne seront pas détaillées.
				\paragraph*{}
			...
			\subsection{Résultats}
				...
				\paragraph*{}
					La bonne pratique voudrais qu'ils aient réalisé un benchmark de \gls{RWLS} et de \gls{RWLS}-R sur les instances \instance{RAIL} afin de montrer que \gls{RWLS}-R obtient de meilleurs résultats et ainsi justifier le développement d'un algorithme différent pour ces instances. Cependant ils ont uniquement réalisé un benchmark de \gls{RWLS}-R, mon avis personnel est qu'ils ont dû implémenter \gls{RWLS} d'une manière très proche de leurs algorithmes et, comme ils précisent que leur machine de test n'est équipé que de 4 Go de RAM, ils n'avaient, comme expliqué dans la section précédente, pas assez de RAM pour résoudre les instances \instance{RAIL}.
				\paragraph*{}
